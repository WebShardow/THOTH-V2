...
Starts PostgreSQL database and development server
"""
...
def start_dev_server():
    ...
    try:
        ...
        if result.returncode != 0:
            print("Error: pnpm not found. Please install pnpm first:")
            print("npm install -g pnpm")
            sys.exit(1)
        
        print(f"Using pnpm version: {result.stdout.strip()}")
        
        # Start pnpm run dev
        process = subprocess.Popen('pnpm run dev', 
                                 cwd=os.getcwd(),
                                 shell=True,
